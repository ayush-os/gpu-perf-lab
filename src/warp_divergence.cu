...
{
    int warp_pos = threadIdx.x % WARP_SIZE;
    int branch = warp_pos % divergence_factor;
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
...
void run_warp_divergence()
{
    ...
    std::vector<int> div_factors = {2, 4, 8, 16, 32};
    for (int divergence_factor : div_factors)
    {
        // Warmup
        partial_divergence<<<NUM_BLOCKS, BLOCK_SIZE>>>(d_output, N, divergence_factor);
        cudaDeviceSynchronize();
        ...
        auto partial_div_stats = BenchmarkStats::compute(times);
        print_results("partial_divergence", partial_div_stats.median * 1000.0f, baseline_ms, divergence_factor);
        times.clear();
    }
